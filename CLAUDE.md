...
- `booking-research` — runs as a cloud routine (`booking-research-business-hours`, `trig_01TMhbXNeURS5xUVqZLRM3CE`) on the hour, 7am-3pm Central Monday-Friday, catches new "Strategy Session with Icarus" bookings on the calendar, researches the lead/company via Perplexity, drafts a personal pre-call text in Bryce's voice (interpretive addition, no SOP covers this window), emails Bryce both (Slack was the original plan, switched to email since Icarus's Slack is company-owned and Bryce isn't admin). **Blocked on Perplexity MCP authorization** (not connected yet as of 2026-08-15, Bryce deprioritized it) — runs fine without it but only logs bookings as pending until connected.
