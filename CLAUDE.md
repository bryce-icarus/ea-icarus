...
- `booking-research` — runs as a cloud routine (`booking-research-business-hours`, `trig_01TMhbXNeURS5xUVqZLRM3CE`) on the hour, 7am-3pm Central Monday-Friday, catches new "Strategy Session with Icarus" bookings on the calendar, researches the lead/company via Perplexity, emails Bryce the brief (Slack was the original plan, switched to email since Icarus's Slack is company-owned and Bryce isn't admin). **Blocked on Perplexity MCP authorization** (not connected yet as of 2026-08-15) — runs fine without it but only logs bookings as pending until connected.
