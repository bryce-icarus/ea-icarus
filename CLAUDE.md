...
- `booking-research` — runs as a cloud routine (`booking-research-business-hours`, `trig_01TMhbXNeURS5xUVqZLRM3CE`) on the hour, 7am-3pm Central Monday-Friday, catches new "Strategy Session with Icarus" bookings on the calendar and emails Bryce a tailored personal text to send the lead (interpretive addition, no SOP covers this window). Deliberately lightweight, does NOT duplicate ClickUp's Discovery Call Prep Assistant. Uses `WebSearch` (not Perplexity, dropped) for a quick skim-level pass only when the booking notes themselves aren't enough. Fully functional now, no pending dependencies.
- `day1-followup-auto` — companion cloud routine (`day1-followup-business-hours`), same hourly/business-hours schedule as `booking-research`. Watches Gmail for new Gemini notes/read.ai transcripts and automatically runs the Day-1 follow-up sequence (recap Gmail draft, text + voice note staged on a high-priority Close task, confirmation note) the moment a call is confirmed, no need to ask. Still never sends anything itself, texts/voice notes stay manual per the SOP. Dedupes against `context/day1-followup-log.md` and against manual `lead-follow-up` runs.
